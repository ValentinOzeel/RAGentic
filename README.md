...
**RAGentic** is an interactive AI assistant embedded within a **Retrieval-Augmented Generation** (_RAG_) system built with Langchain and Qdrant for high-performance vector database management. This application built weith Taipy allows users to engage with their data through a _conversational AI interface_, offering detailed responses to user queries based on _retrieved documents content and chat history_.          
...
• Local SQLite database for organizing and storing structured data.

• Local embedding model (infgrad/stella_en_400M_v5) and vector database (Qdrant) for efficient document retrieval.

• Local LLM through ollama

• Users can seamlessly manage their data (stored in specific SQLite and Qdrant databases) by _uploading new documents_ (Manual text entry, OCR, Text files, PDFs) directly into the application and organizing them with **user-defined tags** for easy retrieval.    

• Filter and query data using custom tags, dates, or search parameters.

• Utilize Retrieval-Augmented Generation (RAG) with customizable LLM and retrieval parameters.

• Accurate Document Citation (through prompt engeenering): Cite retrieved document IDs and the specific documents used to generate responses, ensuring transparency and traceability of information sources.

## Prompts

**Prompt used for generating multi-queries (RAG fusion)**: """
...
_language_: **Python**   
_Frontend_: **Taipy** for gathering user's options and inputs as well as for building the conversational LLM interface.   
_Backend_: **Langchain** as the RAG framework for vector database and retrieval mechanisms.
_Database_: **Qdrant database** for storing user's embedded nodes and SQLite for user/data interactions.   
_LLM_: Choose LLM to run via ollama
_Embeddings model_: **infgrad/stella_en_400M_v5**
...
Clone this repository:
        ...
        git clone https://github.com/ValentinOzeel/RAGentic.git
        cd RAGentic
